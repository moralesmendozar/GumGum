...
device <- list %>% as.tbl_json %>%
  enter_object("auction") %>%
  enter_object("dev") %>%
  spread_values(device = jstring("bti"))
...
full <- foo %>%
  ...
  left_join(device, by="document.id") %>%
  left_join(site, by="document.id") %>%
  rename(Auction=document.id, BidReq=array.index)
full <- full %>%
  select(-reqID)
full <- full[c("Auction","country","region","dayofweek","minute", "hour","month", "date",
               "margin","tmax","bluekai","device","sitetype","sitecat","pubID",
               "BidReq","dsp","vertical",
